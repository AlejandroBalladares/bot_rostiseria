module WebTemplate
  class App < Padrino::Application
    ...
    get '/' do
      "Equipo Praga #{Version.current}"
    end

    post '/reset', :provides => [:js] do
      if ENV['ENABLE_RESET'] == 'true'
        repartidor_repo.delete_all
        calificacion_repo.delete_all
        cliente_repo.delete_all
        pedidos_repo.delete_all
    ...
    put '/clima', :provides => [:js] do
      if ENV['HABILITAR_CLIMA_API_MOCK'] == 'true'
        body ||= request.body.read
        params = JSON.parse(body).symbolize_keys
        ENV['USAR_CLIMA_API_MOCK'] = params[:habilitar].to_s
        unless params[:lluvia].nil?
          ENV['LLUVIA_CLIMA_API_MOCK'] = params[:lluvia].to_s
        end

        status 200
        {message: 'mock clima seteado ok'}.to_json
      else
        status 403
        {message: 'mock clima no habilitado'}.to_json
      end
    end
